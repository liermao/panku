...
const defaultCareTeamImages = getDefaultRuntimeConfig().images.careTeam
...
export default {
  ...
  data() {
    return {
      swiper: null,
      size: 4,
      swiperList: toSwiperList(defaultCareTeamImages)
    }
  },
  async mounted() {
  ...
  methods: {
    async applyRuntimeImages() {
      ...
      this.swiperList = toSwiperList(runtime?.images?.careTeam || defaultCareTeamImages)
    },
